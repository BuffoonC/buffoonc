...
def draw_cluster(datas, extracted_data, labs, num, dic_colors=None, dic_markers=None, name=None):
    plt.cla()
    for k in range(num):
    ...
    plt.scatter(extracted_data[:, 0], extracted_data[:, 1], s=16., color='k', marker='x')
    ax = plt.gca()
...
def remove_data_points_by_index(data, indices_to_remove):
    extracted_data = data[indices_to_remove]
    data_after_removal = np.delete(data, indices_to_remove, axis=0)
    return data_after_removal, extracted_data
...
if __name__ == "__main__":
    ...
    ti_avg = ti_average(rho)
    ti_standd = standdeviation(rho)
    ti_zscore = get_z_score(rho)
    confi_l = (ti_average(ti_zscore) - 1.1 * get_variance(ti_zscore)) * ti_standd + ti_avg
    noise = find_noise(rho, confi_l)
    filtered_data, extracted_data = remove_data_points_by_index(normalized_data, noise)
    dists = squareform(pdist(filtered_data))
    dc = CutoffDistance(dists)
    dc = 0.5 * dc
    rho = get_density(dists, dc)
    higher_deltas, nearest_higher_neiber = get_maxdeltas(dists, rho)
    ...
    confi_h = (ti_average(ti_zscore) + 5 * get_variance(ti_zscore)) * ti_standd + ti_avg
    ...
    result_adjacent_edges_matrix = calculate_adjacent_edges_matrix(filtered_data, labs, dc)
    ...
    draw_cluster(filtered_data, extracted_data, updated_labels, num_merged_clusters, dic_colors, dic_markers, name=file_name + "_cluster_GDPC.svg")
    delnoise_data, extracted_data = remove_data_points_by_index(true_labels, noise)
    ari = adjusted_rand_score(delnoise_data, updated_labels)
    print(f'调整兰德指数（ARI）: {ari:.4f}')
    nmi = normalized_mutual_info_score(delnoise_data, updated_labels)
    print(f'归一化互信息（NMI）: {nmi:.4f}')
    fmi = fowlkes_mallows_score(delnoise_data, updated_labels)
